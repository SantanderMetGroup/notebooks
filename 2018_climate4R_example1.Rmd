...
This worked example contains the full code that reproduces the 1rst example of the paper "climate4R: An Ecosystem of R packages for Climate Data Access, Post-processing and Bias Correction" (Sec. 5 of the manuscript). Here, the same titles are used for the main sactions. These are divided in additional subsections to help with the understanding of the different code chunks. All operations hereinafter are performed with the core packages of climate4R, excepting package installation and the cration of color palletes, for which packages `devtools` and `RColorBrewer` are used respectively.
...
As described in the paper, the SU index (summer days) can be obtained on-the-fly by loading maximum temperature data with function `loadGridData` and by the following argument settings: `aggr.m = "sum"`, `condition = "GT"` and `threshold = 25`. First we load E-OBS observational data by pointing to a NetCDF file via OPeNDAP. Previuous to loading function `dataInventory` might be applied for an overview of the dataset, which returns an inventory of the available variables names, units, coordinates, etc.  
...
In this case, the NetCDF file contains maximum temperature data named as "tx", thus, we set `var = "tx"` in `loadGridData`:
...
In order to load and work with harmonized data we can repit the above operation using a dictionary file, that defines the necessary name and unit transformations to the standard parameters. Function `C4R.vocabulary` displays the climate4R standard vaiable naming and units:

```{r showvocabulary, message=FALSE, warning=FALSE}
C4R.vocabulary()
```

In this case, the only non-standard thing in the E-OBS dataset is the variable name ("tx"), however, we could perform further loading requests using the standard name if a dictionary file is crated previuously (see the [`loadeR` wiki](https://github.com/SantanderMetGroup/loadeR/wiki/Harmonization)). This can be done easily, for instance, in the following manner:
...
Next the loading operation is repeated but using the standard name for the maximum temperature (`var = "tasmax"`) by passing the path to our *.dic file ("dicEOBS.dic") in argument `dictionary`:
...
Next, projection data (both the historical and the RCP8.5 scenarios) is loaded from local NetCDF files, which correspond to a particular RCM (Regional Climate Model ICHEC-EC-EARTH_r12i1p1_SMHI-RCA4_v1) from EURO-CORDEX. These files were downloaded from ESGF (see Appendix A in the manuscript) and stored locally. Next we list them in objects `dir` and `dirf`, the firs corresponding to the historical scenario and the second to the future RCP8.5.
...
dir <- "/oceano/gmeteo/DATA/ESGF/DATASETS/CORDEX/output/EUR-44/SMHI/ICHEC-EC-EARTH/rcp85/r12i1p1/RCA4/v1/day/tasmax/"
dirf <- "/oceano/gmeteo/DATA/ESGF/DATASETS/CORDEX/output/EUR-44/SMHI/ICHEC-EC-EARTH/historical/r12i1p1/RCA4/v1/day/tasmax/"

list.files(dir, recursive = T)
```

Each file in the list contains data for a 5-year period of the same variable (tasmax). Therefore, we use a "catalog" (*.ncml file) to load data for the desired period without worriyng about the different files that need to be read and binded. Next we create two catalogs (for each scenario) with function `makeAggregateDataset` ("CDX_hist.ncml" and "CDX_rcp85.ncml"):
...
Contrarily to the case of the E-OBS dataset, the variable name is standard but not the units (K). Therefore we define the harmonization parameters in another dictionary file ("dicCDX.dic"), where the offset is -273.15 to convert the data to the standar units (ºC):
...
The same operations of annual aggragation and visualization shown before are repeated next. As a result Figure \ref{fig:fig2b} is obtained (Fig. 2b in the manuscript).
...
As can be noted in Figure \ref{fig:fig2b}, the spatial grid of CORDEX is different from E-OBS (Figure \ref{fig:fig2a}). We can use functions `interpGrid` to perform interpolation of CORDEX data to the E-OBS spatial grid as follows. This allows to compute the SU bias of CORDEX data w.r.t E-OBS in the reference period (1971-2000), for which function `gridArithmetics` is used. 

Despite not being necessary, here we apply a land mask before calculating the bias in order to eliminate the values projected by the CORDEX model over the sea. To do so, `gridArithmetics` might be used, first to create the mask and second to apply it.
...
Next, we repeat the same operations of data loading and transformation but for the RCP8.5 scenario and future period 2071-2100.
...
In this case the application of gridArithmetics gives the porjected climate change signal w.r.t the historical data (object `CCsignal`).
...
Next the "additive" type of the "scaling" method is applied to bias correct future montly CORDEX data (object `SUf`) by means of function `biasCorrection`. The output is annually aggregated (object `SUf.bc.annual`) and the climate change signal is again calculated from the bias corrected data (object `CCsignal.bc`). 
...
Other useful plotting function is `temporalPlot` that displays temporal series for multiple datasets
and periods on the same plot, here the SU annual index for E-OBS and CORDEX (raw and bias corrected) and for reference and future periods (1971-2000 and 2071-2100). This function is based on `lattice` and arguments from function `xyplot` are optianlly passed to argument `xyplot.custom`, allowing for a fine tuning of multiple graphical parameters.
