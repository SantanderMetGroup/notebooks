...
function called `RFScore`. In `RFScore` is possible to assign arbitrary
...
A few auxiliary functions have been prepared for paper calculations. These are
next sourced from the `utils` directory:

```{r}
###load of auxiliary functions
...
After completing the calculation of the series calibrated using the adaptive
methodology, it is crucial to compute a series of standard calibrations
to evaluate their performance and compare them with the adaptive methodology. 
For this purpose, the `downscaleR` package includes the `biasCorrection` function,
which provides specific arguments depending on the calibration method selected.
...
1.  The `multiplicative` argument determines the type of scaling method
    ...
    temperature or `multiplicative` for (lower)bounded variables like
...
2.  The `fitdistr.args` argument allows us to specify additional
    ...
    `pqm` method to account for the different theoretical distribution
    function fitting.
