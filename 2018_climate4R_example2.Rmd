...
For example, if we are interested in loading observations from the **E-OBS** dataset at 0.25 degrees resolution we can filter the names returned by `UDG.datasets` passing an appropriate pattern to function `grepl`:
...
Note that function `spatialPlot` recognizes a multi-member grid and displays a map for each member. For instance, next we plot the ensemble of the RCP8.5 scenario to generate Figure \ref{fig:fig8a} (Fig. 8(above) in the manuscript)):
...
In order to calculate the bias of each model with respect to the observations and generate Figure \ref{fig:fig8b} (Fig. 8(below) in the manuscript) we apply function `aggregateGrid`, `gridArithmetics`, `bindGridMember` and `spatialPlot` as follows:
...
Next the raw SU index (summer days) is calculated for the whole ensemble (object `SUf.ens`) and future period 2071-2100, in a single line with function `climdexGrid`:
...
And finally, we plot the results to generate Figures \ref{fig:fig6above1} and \ref{fig:fig6above2} (Figs. 6(above, left) and 6(above, right) in the manuscript:
...
We also extract the nearest grid box to Zaragoza for E-OBS (object `SU.Z.eobs`) and CORDEX (object `SU.Z.cdx`). The 1rst memeber of CORDEX is also extracted and everything is listed in object `SU.Z` for plotting:
...
This section replicates the procedures shown above but for calculating a precipitation based index, i.e. CDD (consecutive dry days). Function `climdexShow` displays an overview of the available ETCDDI indices:
