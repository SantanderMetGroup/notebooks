...
```{r}
variables <- c("psl","tas","ta@500","ta700","ta850","hus@500","hus@850","z@500")
...
```{r}
xh <- lapply(variables, function(x) {
  loadGridData(dataset = "CMIP5_EC-EARTH_r12i1p1_historical",
               var = x,
               lonLim = c(-10,32),
               latLim = c(36,72),
               years = 1979:2008)
}) %>% makeMultiGrid() %>% interpGrid(new.coordinates = getGrid(x.eur))
```
...
```{r}
xf <- grid.list <- lapply(variables, function(x) {
